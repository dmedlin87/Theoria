...
- **References**: `test_results.txt` for the failure trace and the seeding logic in `theo/infrastructure/api/app/db/seeds.py`.
...
- **References**: Failure details in `test_results.txt` and existing guard implementations in `theo/infrastructure/api/app/ingest/network.py`.
...
- **References**: The failing assertion in `test_results.txt` and the inflight ledger logic in `theo/infrastructure/api/app/ai/ledger.py`.
