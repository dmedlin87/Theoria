...
* **Dependencies** – Installs succeed without scikit-learn thanks to bundled stubs, but installing the ML extra unlocks
  histogram gradient boosting (`pip install ".[ml]" -c constraints/ml.txt`).
...
| Ranking CLI slow path | `pip install ".[ml]" -c constraints/ml.txt && pytest -m slow -k reranker_scripts` |
