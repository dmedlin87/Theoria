...
def ensure_crypto_available() -> None:
    if x509 is None or rsa is None:
        raise RuntimeError(
            "The 'cryptography' package is required to generate certificates. Install dependencies with 'pip install .[dev] -c constraints/guardrails.txt -c constraints/dev.txt'."
        )
