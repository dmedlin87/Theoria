...
for _module_name in (
    ...
    _stubbed_modules[_module_name] = sys.modules.pop(_module_name, None)

_real_discoveries = importlib.import_module("theo.services.api.app.discoveries")
RealDiscoveryService = _real_discoveries.DiscoveryService
routes_discoveries = importlib.import_module(
    "theo.services.api.app.routes.discoveries"
)


@pytest.fixture(scope="module", autouse=True)
def _use_real_discovery_service() -> Generator[None, None, None]:
    original_service = getattr(routes_discoveries, "DiscoveryService", None)
    original_response = getattr(routes_discoveries, "DiscoveryResponse", None)

    class _CoercingDiscoveryResponse(routes_discoveries.DiscoveryResponse):  # type: ignore[misc]
    ...
    routes_discoveries.DiscoveryService = RealDiscoveryService
    routes_discoveries.DiscoveryResponse = _CoercingDiscoveryResponse
    try:
        yield
    finally:
        if original_service is not None:
            routes_discoveries.DiscoveryService = original_service
        else:
            delattr(routes_discoveries, "DiscoveryService")
        if original_response is not None:
            routes_discoveries.DiscoveryResponse = original_response
        else:
            delattr(routes_discoveries, "DiscoveryResponse")
