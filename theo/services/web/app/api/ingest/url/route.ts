...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const response = await fetchWithTimeout(`${baseUrl}/ingest/url`, {
      method: "POST",
    ...
    return createProxyErrorResponse({
      error,
      logContext: "Failed to proxy URL ingestion request",
      message: "URL ingestion service is currently unavailable. Please try again later.",
      status: 502,
    });
  }
}
